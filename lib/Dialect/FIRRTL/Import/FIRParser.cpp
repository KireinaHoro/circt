...
/// Return a nullable Value that is, optionally, the port that this
/// value originates from.
static void getPorts(Value value, SmallVectorImpl<BlockArgument> &ports) {
  // Base case: the value was defined in a block and must be a port.
  auto blockArg = value.dyn_cast_or_null<BlockArgument>();
  if (blockArg) {
    ports.push_back(blockArg);
    return;
  }

  TypeSwitch<Operation *>(value.getDefiningOp())
      // Recurse into subaccess input and index.
      .Case<SubaccessOp>([&](auto a) {
        getPorts(a.input(), ports);
        getPorts(a.index(), ports);
      })
      // Recurse across sufield and subindex.
      .Case<SubfieldOp, SubindexOp>(
          [&](auto a) { getPorts(a.input(), ports); });
  return;
}

/// Return a nullable subfield Value that is, optionally, used to
/// reference an instance if the input Value points at an instance.
...
static Kind getKind(Value value) {
  ...
  TypeSwitch<Operation *>(value.getDefiningOp())
      ...
      .Case<RegInitOp, RegOp, WireOp>([&](auto a) { kind = InOutKind; })
...
Value FIRStmtParser::convertToPassive(Value input, Location loc) {
  ...
  SmallVector<BlockArgument, 4> ports;
  getPorts(input, ports);
  for (auto port : ports)
...
ParseResult FIRModuleParser::parseModule(unsigned indent) {
  ...
  for (auto a : moduleContext.rhsSinks) {
    ...
    // Add the wire, drive the sink, and replace all uses of the RHS
    // sink with the wire
    LocWithInfo info(getToken().getLoc(), this);
    Value aWire =
    ...
    // TODO: This leaves some unused operands that will be cleaned up
    // later. This could be cleaned up here.
    if (subfieldOption) {
